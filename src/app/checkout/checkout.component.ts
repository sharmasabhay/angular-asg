...
export class CheckoutComponent implements OnInit {
        ...
        this.dataService.singaporeTimezone(params)
          .subscribe(
            (data) => {
                ...
                let next_day_arr = [], same_day_arr = [], two_hours_arr = [], store_hours_arr = [];

                for(var i in slotData.next_day){
                    next_day_arr.push(Object.assign({ day: i }, slotData.next_day[i]));
                }

                for(var i in slotData.same_day){
                    same_day_arr.push(Object.assign({ day: i }, slotData.same_day[i]));
                }

                for(var i in slotData.two_hrs){
                    two_hours_arr.push(Object.assign({ day: i }, slotData.two_hrs[i]));
                }

                for(var i in slotData.store){
                    store_hours_arr.push(Object.assign({ day: i }, slotData.store[i]));
                }

                this.next_day = next_day_arr;
                this.same_day = same_day_arr;
                this.two_hours = two_hours_arr;
                this.store_hours = store_hours_arr;
            }
        );
    }
    ...
    setNewSlot(slotbunch,slot,skey) {
        this.activeDay = slotbunch.datekey;
        this.activeSlot = skey;        
        this.selectedTimeSlot = new Date(slotbunch.datekey*1000);
        this.scheduledDelivery = 'NEXT DAY delivery';
        this.between = 'between';
        this.onlyManual = false;
        this.checkout.date = this.selectedTimeSlot;
